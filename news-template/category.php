...
<div class="post-container">
    ...
    if(isset($_GET["cat"])){
     ...
     $pagenation = "select * from category where category_id='$cat'";
            $resultPage = mysqli_query($conn,$pagenation) or die("category Error");
            $rowDa      = mysqli_fetch_assoc($resultPage);
       
    ?>
  <h2 class="page-heading"><?php echo $rowDa["category_name"];?></h2>
    <?php
        include "config.php";
        if(isset($_GET["cat"])){
            $cat     = $_GET["cat"];
        ...
        $querySelect = "select p.post_id,p.title,p.description,p.category,p.author,
            ...
            where p.category=$cat order by p.post_id DESC limit $offest,$limits";
                ...
                if(mysqli_num_rows($resultPage)){
                   ...
                   // $totalRecoreds = mysqli_num_rows($resultPage);
                    ...
                    for($i=1;$i<=$totalPage;$i++){
                       ...
                       echo '<li class="'.$active.'"><a href="category.php?cat='.$cat.'&page='.$i.'">'.$i.'</a></li>';
                       if($totalPage > $page){
                       echo '<li><a href="category.php?cat='.$cat.'&page='.($page+1).'">Next</a></li>';
                       }  
